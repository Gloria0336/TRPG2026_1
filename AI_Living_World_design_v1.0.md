...
### 4.2 數值框架：5e bounded 基礎 ＋ PF2e 五階熟練（混合制，定案）
本專案**刻意採用混合數值框架**：以 5e bounded accuracy 的熟練加值作為基礎成長曲線（壓縮等級差、抑制數值膨脹、低高等仍可同場共事），再疊上 PF2e 的五階熟練表達「同領域精熟度」的區隔。等級差仍有意義，但不像純 PF2e 那樣劇烈；熟練階級越高，同領域越明顯佔優。**此為定案選擇，非 PF2e 原版的 `level + 2/4/6/8`；亦即本專案保留 bounded accuracy 作為基礎，PF2e 只貢獻五階命名與階梯。**

- 角色等級：1–20，為戰力與規則複雜度核心。
- 熟練階級：未受訓／受訓／專家／大師／傳奇（沿用 PF2e 命名與五階結構）。
- **熟練加值算法（定案）**：
  - 基礎熟練加值 `PB = 2 + ⌊(等級 − 1) ÷ 4⌋`（5e 曲線，等級 1–20 對應 +2 ~ +6）。
  - 各階在 PB 之上疊固定階梯：未受訓 ＝ 0（完全不計 PB）、受訓 ＝ PB、專家 ＝ PB + 2、大師 ＝ PB + 4、傳奇 ＝ PB + 6。
  - 範例（角色 3 級，PB ＝ 2）：受訓 +2、專家 +4、大師 +6、傳奇 +8。
  - 能力調整值沿用 `⌊(屬性值 − 10) ÷ 2⌋`（5e／PF2e 相同）。
...
- **B 判定底層**：✅ PF2e 四階成敗與混合熟練加值（5e bounded 基礎 ＋ PF2e 五階，§4.2）／PF2e Encounter ＋ World Threat subsystem（§10）／協力與加值類型（§4.9）／對抗 DC（§4.10）。
...
- **v1.1** — 修訂 §4.2 熟練加值算法：由原「純 PF2e `level + 2/4/6/8`、禁止 bounded accuracy」改為**定案的混合制**——5e bounded 熟練加值 `PB = 2 + ⌊(等級−1)÷4⌋` 為基礎曲線，PF2e 五階在 PB 上疊 0／PB／PB+2／PB+4／PB+6。此為刻意保留 bounded accuracy 的設計選擇，與現行程式 (`app/engine/types.py`) 一致。同步更新 §13 完成度摘要。
