...
| 成長：階級 | 放棄膨脹等級，採 F→E→D→C→B→A→S 冒險者階級，決定任務／地區／資源資格，升階緩慢 | ✅ 見 §12 |
| 成長：來源 | 角色成長主要來自技能／專長／法術／裝備／人脈，避免數值膨脹（技能成長見 §12.4） | ✅ 見 §12 |
...
### 4.1 核心機制：混合制（壓低 d20 ＋ margin 三段帶）
擲 d20 ＋修正對 DC，但結果不二元，依與 DC 的差距切成多段；所有段位由程式計算，AI 只填敘事。

### 4.2 數值框架（bounded accuracy）
壓低膨脹，使 F 階仍能靠人數／工具／環境參與 S 階情境（直接服務「低階高階共玩」）。

- 屬性修正：−1 ～ +4　*(可調)*
- 技能階梯：未訓 +0 ／受訓 +2 ／專精 +4 ／大師 +6　*(可調，與階級掛鉤)*
- 典型總修正：F 階 +2～+4，S 階 +10～+12（全距約 8，對照 PF2e 約 30）

### 4.3 DC 難度錨點（七級，可調）
微不足道 5｜簡單 10｜中等 15｜困難 20｜高難 25｜極限 30｜傳說 35
→ 修正天花板約 +12，故 DC 30+ 單人幾乎不可能，**逼出多解**（協力／工具／環境／消耗資源換加值）。

### 4.4 三段帶切點（相對 DC 的差距）
- **成功**：總值 ≥ DC
- **部分成功**：差 1～4（成功，但附枚舉代價）
- **失敗**：差 5 以上
- **天然 20**：結果升一階（失敗→部分→成功→大成功＝額外好處）
- **天然 1**：結果降一階（→大失敗＝額外代價）

*(替代旋鈕：以 2d6／3d6 取代 d20 可得鐘形分佈，極端結果更少、可信度更高，待定。)*

### 4.5 DC 歸屬（分工 fork 定案）
**程式查表為主、AI 為輔且受限**：常見動作 × 環境由程式查表給 DC；玩家做表外創意行動時，AI 可提案 DC，但**只能落在七個錨點上（5/10/…/35），不能自由給數字**——防話術壓 DC、保一致。
...
玩家自然語言輸入
 ...
 →[程式] 查表 / AI 提案 DC（錨定七級）
 →[程式] 算 modifier（屬性＋訓練＋裝備＋環境＋協力）
 →[程式] 擲 d20 → margin → band → 天然20/1 調整
 →[程式] band → 後果：部分／失敗時，從代價清單選 {類型, 嚴重度}
...
嚴重度由 band 決定（部分成功＝輕～中，失敗＝中～重）。AI 只能選類型＋填敘事，不能自創成敗。

### 4.8 判定範例（撬鎖，混合制）
- 情境：D 階冒險者凜撬當鋪後門。程式：門鎖 DC 15；凜開鎖修正 +5。
- AI 將「我撬鎖」解析為巧手檢定 → 程式擲 d20 = 8 → 8+5 = 13，對 DC 15 差 2 → 落在「部分成功」帶。
- 程式抽代價類型 `工具損耗` → AI 敘述：鎖開了，但最後一根撬針折斷在鎖孔裡，這扇門無法再撬第二次。
- 多解一致性：撬鎖／撞門／賄賂守衛／knock 法術／話術，皆走同一條檢定脊椎，僅替換技能欄位。

### 4.9 協力與外部加值 ✅
- **外部加值總封頂**：協力＋環境＋工具＋資源對 DC 的淨影響上限＝最多抵消兩級 DC（*可調*），保 §4.2 低膨脹與 §4.3 DC 錨點不失效。
- **協力：遞減封頂**：第 1 助 +2、第 2 助 +1、其餘 +0，總封頂 +3（*可調*）。不需協助者擲骰，最 async、天然防疊人破天花板。
- **協力者技能認證**：協助者須在該檢定的 approach 技能達「受訓」以上才提供加值（呼應 §4.2 階梯）——外行幫不上忙（例：未受訓者「幫」撬鎖＝+0）。
- **環境/情境修正**：分級 ±1（輕微）／±2（顯著）／±4（極端），程式查表給。
- **工具加值**：用 §6 `items.modifier`。
- **消耗資源換加值**：花錢/時間/消耗品 → 降一級 DC 或 +固定值，程式定兌換率。

### 4.10 對抗檢定（PvP／NPC 抗性）✅
- **被動方轉靜態 DC**：被動方能力轉成靜態 DC（＝10＋被動 mod），主動方擲、接 §4.4 三段帶。只擲一顆、波動小、好程式判。
- **async 關鍵**：被動方離線也能判（不要求被偷襲的離線角色即時擲防禦骰）。
- **NPC 抗性**沿用同機制（靜態 DC），與對抗檢定統一。
- 結果寫 event_log，致死接 §6/§10 死亡減損；**PvP 社會規則**（需否同意、限定區域、通緝後果）屬 A 制度層，此處只定判定機制。
...
- **兩條成長軸長在同一人身上**：戰力軸＝`actors.rank`（F→S，慢、壓膨脹）；廣度/資歷軸＝`players.seniority_level` ＋ `unlocks`（獎勵遊玩時間，解鎖「能做的事」，不加戰力）。
- **死亡＝進度減損公式（無快照）**：死亡套懲罰公式（降階／損資源），讀 `rank / death_count / assets` 寫回現況表；死亡事件仍寫 `event_log`（世界記得），但不做時間回溯。
- **狀態存法**：核心熱欄位（hp/rank/location/control_mode）＋ JSONB 軟欄位（attributes/skills/assets/flags），迭代不改 schema。
...
### 7.2 離線角色行動：純隨機表
- 機制：加權隨機表選行動（研究／訓練／經商／採集／社交／巡邏…）。
- 權重輸入：actor 的技能／階級／地點／方針旗標（玩家可設方針偏置權重，未設走預設）。
- 一致性：選定行動後**仍走 Resolution Engine**（§4 判定脊椎），結果寫 event_log。
- 代價（已知）：行為較無個性、離線報告偏模板化；換得近乎零 token、最穩、最好實作。
...
- **DC 提案 enum 約束**：AI 提 DC 只能從七錨點（5/10/…/35）選，用 function calling／schema 限定，無法自由給數字。
...
玩家自然語言
 ...
 →[裁判] 表外才提案 DC（七錨點 enum）
 →[Resolution Engine] 算 modifier→擲骰→band→代價→state delta→event_log（§4.6）
...
偵測到需擲骰的檢定 → bot 貼 embed ＋「🎲 擲骰」按鈕
 ...
 → 伺服器擲真骰：nat roll → 修正 → total vs DC → band（§4.4）
 → 貼通用「滾動中」GIF（單則訊息，不逐幀編輯）
 → 單次編輯揭曉：結果 embed（nat roll ＋ 修正拆解 ＋ band）
 → 敘事者接續敘事
```
...
## 10. 戰鬥系統（async） — v0.1 ✅（本次定案）

> B 判定底層的延伸（建在 §4 判定脊椎上）。解 V1.0 標記的「非同步戰鬥」硬問題。
> 根本取捨：**戰術深度 ↔ 非同步自由不可兼得**；本設計以 async 為主幹、同步場為深度逃生口。

### 10.0 模型：③ 進度條主幹 ＋ 同步場（一次到位）
預設走 async 進度條（日常、多人共打、離線可貢獻）；想要戰術深度時開「同步戰鬥場次」跑即時回合。兩套都做（**代價：兩套戰鬥解算/規則/UI ＋ 結果接合，工程量較大**）。

### 10.1 進度條＝一場競賽（接 §7 世界循環）
每個 boss/威脅有兩條 clock：
...
### 10.3 多人貢獻累加
玩家上線做攻擊檢定（§4 脊椎），band 決定削減量（大成功削多、部分成功削少且自身受險），累加進共享討伐進度。可不同時上線、零協調。離線角色用 §7.2 隨機表貢獻。

### 10.4 傷害／死亡（接 §6 死亡減損）
攻擊檢定失敗／部分成功 → 攻擊者 hp delta；hp 歸零 → 觸發死亡減損公式（降階／損資源）＋ 在安全地復活退場，**非刪角**（呼應「玩家=角色、一人一角、死亡非永久」）。死亡寫 event_log。

### 10.5 同步場次：自由約 ＋ 最終決戰門檻
- 平時任何隊伍可自由喬時間開同步突擊（削減討伐進度）。
- 「最終討平 boss」的決戰場須討伐進度達某階段才解鎖 → boss 之死是社群共同累積的成就（呼應 §7 開章＝集體成就）。
- 結果接合：同步場跑即時回合，結算後把淨削減量／勝負**寫回同一條 boss clock ＋ event_log**；即同步場是「一次集中突擊」，機制上仍服務同一威脅。
...
> A 最後一塊。接 §4.2（技能階梯掛階級）、§6（actors.rank）、§10（死亡降階）。

### 12.0 階級 gate（F→E→D→C→B→A→S）
階級決定三種資格：**任務資格**（高階任務需對應階級才能接）、**地區資格**、**資源權限**（高階裝備/設施/情報）。
- 地區：**預設軟 gate**（能進但風險自負，高 DC／強威脅 clock 輾壓低階者）；少數極危險區用硬 gate 直接擋。
- 階級＝戰力/資格軸，與廣度軸（`players.seniority`）分離。

### 12.1 升階：混合（累積 ＋ 晉升試煉）
累積實績/經驗達標 → 解鎖「可挑戰晉升試煉」→ 通過才實際升階。
- 試煉由**公會（NPC 制度）發起認證** → 讓 §11 制度層有了功能（公會＝認證機構），呼應「冒險者公會」設定、防純刷分。
- 試煉走 §4／§10 判定脊椎，async 可跑。

### 12.2 速度：指數遞增
低階快速通過（新手有成長感）、高階極緩（A→S 極難）。呼應「升階緩慢」＋ bounded accuracy——**S 階是傳說級稀少的頂點**，非人人可達。

### 12.3 降階（死亡減損）後升回較快
死亡減損降階後（§6/§10），升回比首次快（保留部分熟練），仍需重新達標——避免一人一角的死亡懲罰過重。

### 12.4 技能成長（平行軸，bounded accuracy 鎖）
技能階梯（未訓→受訓→專精→大師，§4.2）的提升靠**使用/訓練累積 ＋ 消耗資源**（時間/金錢/導師）。
- **階級門檻封頂**：高精熟度受階級限制（例：F 階練不到大師），防止低階取得高階修正、保 §4.2 低膨脹。
...
- **B 判定底層**：✅ 混合制（§4）／async 戰鬥（§10）／協力加值（§4.9）／對抗檢定（§4.10）。
- **C 技術架構**：✅ 拓撲（§5）／資料模型（§6）／世界循環（§7）／AI 編排（§8）／前端分工（§9）。

**下一階段（已非設計決策，屬實作與內容）**
- 實作：依 §5 技術棧搭 core/worker/gateway/web，落 `schema_v0.1.sql`。
- 數值調校：所有標「*可調*」的常數（DC 錨點、加值封頂、tick cadence、離線時長門檻、升階曲線…）需實測校準。
...
## 變更紀錄
- **v0.1** — 收錄判定底層（混合制：壓低 d20 ＋ margin 三段帶），定案分工原則與 DC 歸屬。
- **v0.2** — 收錄技術架構拓撲（全新設計：模組化單體 ＋ worker、事件驅動 event_log、Postgres ＋ pgvector），更新 Roadmap。
- **v0.3** — 收錄資料模型（玩家=角色 1對1、PC/NPC 同表、死亡減損無快照、CQRS-lite event_log、pgvector 記憶層）；完整 DDL 見 `schema_v0.1.sql`。
...
- **v0.7** — 收錄戰鬥系統（async，③ 進度條主幹＋同步場一次到位）：威脅 clock vs 討伐進度競賽、推進靠 tick＋玩家行動、傷害接死亡減損、同步場自由約＋最終決戰門檻。B 剩協力/環境加值、對抗檢定。
- **v0.8** — 收錄 §4.9 協力與外部加值（遞減封頂＋協力者技能認證＋總封頂）、§4.10 對抗檢定（被動方轉靜態 DC，async 友善）。**B 判定底層全部完成**。
- **v0.9** — 收錄 §11 制度優先（治理）：接管混合路徑、結構化政策槽＋decision event、制度代理人離線接手、分級三段防鎖死、勢力模板。§3 玩家掌權/NPC代理人/勢力模板/內容耗盡四列轉 ✅。A 僅剩 F–S 升階。
- **v1.0** — 收錄 §12 階級與升階（混合晉升＝累積＋公會晉升試煉、指數遞增、降階較快升回、§12.4 技能成長 bounded accuracy 鎖）。**A/B/C 三支柱設計全部完成，設計骨架封頂。** 下一階段轉實作與內容。
