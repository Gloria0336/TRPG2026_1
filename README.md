...
請見 `.env` 與 `app/config.py`：`DICE_SEED`（確定性骰子）、`NARRATE_CONTEXT_WINDOW`、`AI_OFFLINE`、模型 ID，以及 dashboard / portal 的 host／port。DC 錨點位於 `app/engine/rules_5e.py`；劇本與 stat blocks 位於 `app/content/`。
